...
var advertisementParams: Params.advertising,
  app,
...
export default class BluetoothService implements Services.BluetoothService {
  ...
  startAdvertising(params: Params.advertising) {
    ...
    const self = this;
    return new Promise<{ status: Services.Status }>(
      (resolve: any, reject: any) => {
        self.isAdvertising().then((result: { status: boolean }) => {
          if (!result.status) {
  ...
  requestPermission() {
    return new Promise((resolve, reject) => {
      bluetoothle.requestPermission((result, ...rest) => {
        result ? resolve({ result, rest }) : reject({ result, rest });
      });
    });
  }
  startScan(onData: Function, onErr: Function) {
    const self = this;
    return (params: BluetoothlePlugin.ScanParams) => {
      self
        .initialize()
        .then(() => self.requestPermission())
        .then((data, ...r) => {
          if (!data) return "user denied perms";
          bluetoothle.startScan(
            (result, ...rest) => {
              console.log("made it to starscan callback", result);
              onData(result, result.address);
            },
            (err, ...rest) => ({ err, rest }),
            params
          );
        })
        .catch((e, ...rest) => onErr({ e, rest }));
    };
  }
  stopScan() {
    return new Promise((resolve, reject) => {
      bluetoothle.stopScan(result => resolve(result), err => reject(err));
    });
  }
}
